// Sun and moon visibility: one shadow ray per pixel toward a random point on
// the light's disc (widened for soft contact shadows), accumulated over
// frames. History is reused only where the previous frame saw the exact same
...
fn light_visible(s: Surface, dir: vec3<f32>) -> f32 {
    if dot(s.face, dir) <= 0.0 {
        return 0.0;
    }
    let hit = march(secondary_ray(s, dir, 1500.0));
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if (id.w >> 30u) == 0u {
        textureStore(out_vis, pixel, vec4<f32>(1.0, 1.0, 0.0, 1.0));
        return;
    }
    ...
    let alpha = 1.0 / n;
    let accumulated = mix(prev.rg, vec2<f32>(sun_vis, moon_vis), alpha);
    textureStore(out_vis, pixel, vec4<f32>(accumulated, 0.0, n));
}
