...
// all higher orders as the geometric series 1 / (1 - f_ms). Built once.
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    for (var i = 0; i < sqrt_samples; i++) {
        for (var j = 0; j < sqrt_samples; j++) {
            ...
            for (var s = 0; s < steps; s++) {
                ...
                luminance += throughput * (in_scatter - in_scatter * step_t) / max(m.extinction, vec3<f32>(1e-6));
                throughput *= step_t;
            }
            if t_ground > 0.0 {
    ...
    // Averaged over the sphere with an isotropic phase, second_order is both
    // the second-order in-scattering and the transfer ratio r of each further
    // bounce, so all orders sum to r / (1 - r).
    let r = clamp(second_order, vec3<f32>(0.0), vec3<f32>(0.99));
    let psi = r / (1.0 - r);
