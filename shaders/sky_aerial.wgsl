// Aerial perspective froxels, per frame: in-scattering (rgb, per unit sun
// illuminance) and mean transmittance (a) from the camera to each slice.
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let result = integrate_scattering(origin, world_dir, normalize(frame.sun_dir), distance_km, steps, transmittance_lut, multiscatter_lut, lut_sampler);
